...
with device:
    ...
    # for RO_idx in range(RO):
        ...
        for r in range(kdat_temp.shape[0]): #loop over CEST Repititions
            ...
            kdat_temp[r,0,...]=sp.mri.app.L1WaveletRecon(kdat_temp[r,...], mps, lamda=args.r, max_iter=args.i, device=device, show_pbar=False).run().get()
        ...
        if RO_idx==RO_start:
            with h5py.File(DATA_DIR+f'CEST_CS_recons_{Rep}_r_{args.r}_i_{args.i}_mapc_{str(mps_file).split("_")[-1].rsplit(".",1)[0]}.h5','w') as f:
                f.create_dataset(f'CEST_recon_RO_idx_{RO_idx}',data=recon,chunks=chunks)
        else:
            with h5py.File(DATA_DIR+f'CEST_CS_recons_{Rep}_r_{args.r}_i_{args.i}_mapc_{str(mps_file).split("_")[-1].rsplit(".",1)[0]}.h5','a') as f:
                f.create_dataset(f'CEST_recon_RO_idx_{RO_idx}',data=recon,chunks=chunks)
        del recon
